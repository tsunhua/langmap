#!/usr/bin/env python3
"""Repair dictionary edges whose old import attached examples to headwords.
...
* equivalent pairs and standalone example pairs use the mapping bit;
...
For already-reviewed legacy claims that are absent from the current source export,
use ``--only-remove-unmatched-edge`` with one or more explicit edge IDs.  This
avoids rescanning a large mounted JSONL while keeping the mirror-side source claim
check and an auditable report.
"""
...
def _iter_claims_for_edges(
    connection: sqlite3.Connection, source_id: int, edge_ids: list[int]
) -> Iterable[EdgeClaim]:
    if not edge_ids:
        return
    ...
    query = f"""
        SELECT
          ...
          lb.code AS right_code,
          eb.text AS right_text,
          (SELECT COUNT(*) FROM expression_edge_sources all_es WHERE all_es.edge_id=es.edge_id) AS source_claim_count,
          (SELECT COUNT(DISTINCT all_es.source_id) FROM expression_edge_sources all_es WHERE all_es.edge_id=es.edge_id) AS source_count
        FROM expression_edge_sources es
        JOIN expression_edges e ON e.id = es.edge_id
        JOIN expressions ea ON ea.id = e.expression_a_id
        JOIN languages la ON la.id = ea.language_id
        JOIN expressions eb ON eb.id = e.expression_b_id
        JOIN languages lb ON lb.id = eb.language_id
        WHERE es.source_id=? AND es.edge_id IN ({placeholders})
        ORDER BY es.edge_id, es.source_marker
    """
    for row in connection.execute(query, (source_id, *edge_ids)):
        left = (str(row["left_code"]).lower(), canonicalize_text(str(row["left_text"])))
        right = (str(row["right_code"]).lower(), canonicalize_text(str(row["right_text"])))
        yield EdgeClaim(
            edge_id=int(row["edge_id"]),
...
def _write_sql(
    ...
    *,
    ...
    jsonl_sha256: str,
    approved_unmatched: bool = False,
) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "-- Generated by scripts/dictionary/repair_example_edges.py.",
        f"-- source_id={source_id}; jsonl_sha256={jsonl_sha256}",
        "-- Standalone example pairs are ordinary mappings; no headword edge is created.",
        "-- Explicitly approved unmatched legacy claims are removed below."
        if approved_unmatched
        else "-- Unmatched legacy claims are intentionally preserved for review.",
    ]
    ...
    for chunk in _chunks(sorted(set(source_only_example_edges))):
        lines.append(
            "UPDATE expression_edges SET relation_mask = CASE WHEN (relation_mask & 3) = 0 THEN 1 ELSE (relation_mask & 3) END "
...
def repair(
    ...
    *,
    ...
    remove_unmatched_edges: Iterable[int] = (),
) -> dict[str, Any]:
    ...
    approved_unmatched_edge_ids = sorted(set(int(edge_id) for edge_id in remove_unmatched_edges))
    unmatched_by_edge = {claim.edge_id for claim in unmatched_claims}
    missing_approved = [
        edge_id for edge_id in approved_unmatched_edge_ids
        if edge_id not in unmatched_by_edge
    ]
    if missing_approved:
        raise ValueError(
            "approved unmatched edge is not an unmatched source claim: "
            + ", ".join(str(edge_id) for edge_id in missing_approved)
        )
    approved_unmatched_claims = [
        claim for claim in unmatched_claims
        if claim.edge_id in approved_unmatched_edge_ids
    ]
    unmatched_claims = [
        claim for claim in unmatched_claims
        if claim.edge_id not in approved_unmatched_edge_ids
    ]
    ...
    _write_sql(
        ...
        stale_edges=sorted(set(polluted_edge_ids + approved_unmatched_edge_ids)),
        stale_claim_edges=sorted(set(polluted_edge_ids + approved_unmatched_edge_ids)),
        jsonl_sha256=jsonl_sha256,
        approved_unmatched=bool(approved_unmatched_edge_ids),
    )
    payload: dict[str, Any] = {
        ...
        "approved_unmatched_edge_count": len({claim.edge_id for claim in approved_unmatched_claims}),
        "approved_unmatched_edges": _sample(approved_unmatched_claims),
...
def repair_approved_unmatched_edges(
    jsonl: Path,
    mirror: Path,
    output: Path,
    report: Path,
    ...
    *,
    ...
    """Remove only explicitly reviewed legacy claims without a full source scan."""

    approved_ids = sorted(set(int(edge_id) for edge_id in edge_ids))
    if not approved_ids:
        raise ValueError("at least one approved unmatched edge is required")
    with jsonl.open("r", encoding="utf-8-sig") as handle:
        first_line = next((line for line in handle if line.strip()), "")
    try:
        header = json.loads(first_line)
    except json.JSONDecodeError as exc:
        raise ValueError(f"invalid JSONL dictionary header: {jsonl}") from exc
    if not isinstance(header, dict) or header.get("record_type") != "dictionary":
        raise ValueError(f"missing Structured JSONL v2 dictionary header: {jsonl}")
    source_key = str(header.get("dictionary_key", ""))
    if dictionary_key is not None and source_key != dictionary_key:
        raise ValueError(f"dictionary_key mismatch: header={source_key!r}, requested={dictionary_key!r}")
    connection = _connect(mirror)
    try:
        source_id = _source_id(connection, source_key)
        claims = list(_iter_claims_for_edges(connection, source_id, approved_ids))
    finally:
        connection.close()
    found_ids = {claim.edge_id for claim in claims}
    missing = [edge_id for edge_id in approved_ids if edge_id not in found_ids]
    if missing:
        raise ValueError(
            "approved unmatched edge is not a source claim in mirror: "
            + ", ".join(str(edge_id) for edge_id in missing)
        )
    jsonl_sha256 = hashlib.sha256(jsonl.read_bytes()).hexdigest()
    _write_sql(
        output,
        source_id=source_id,
        source_only_example_edges=[],
        shared_example_edges=[],
        valid_equivalent_edges=[],
        stale_edges=approved_ids,
        stale_claim_edges=approved_ids,
        jsonl_sha256=jsonl_sha256,
        approved_unmatched=True,
    )
    payload: dict[str, Any] = {
        "schema_version": 1,
        "approval_mode": "explicit-unmatched-edge",
        "jsonl": str(jsonl),
        "jsonl_sha256": jsonl_sha256,
        "dictionary_key": source_key,
        "source_id": source_id,
        "source_entry_count": int(header.get("entry_count", 0)),
        "approved_unmatched_edge_count": len(approved_ids),
        "approved_unmatched_edges": _sample(claims, limit=len(claims)),
        "source_claim_count_checked": len(claims),
        "source_claims_not_scanned": True,
    }
    report.parent.mkdir(parents=True, exist_ok=True)
    report.write_text(
...
def main() -> int:
    ...
    parser.add_argument(
        "--remove-unmatched-edge",
        dest="remove_unmatched_edges",
        action="append",
        type=int,
        default=[],
        help="明確批准移除一條已核實的 unmatched legacy source claim；可重複指定",
    )
    parser.add_argument(
        "--only-remove-unmatched-edge",
        action="store_true",
        help="只核實並移除 --remove-unmatched-edge 指定的 source claims，不掃描整份 JSONL",
    )
    args = parser.parse_args()
    if args.only_remove_unmatched_edge:
        if not args.remove_unmatched_edges:
            parser.error("--only-remove-unmatched-edge requires --remove-unmatched-edge")
        payload = repair_approved_unmatched_edges(
            args.jsonl,
            args.mirror,
            args.output,
            args.report,
            args.remove_unmatched_edges,
            dictionary_key=args.dictionary_key,
        )
    else:
        payload = repair(
            args.jsonl,
            ...
            args.report,
            dictionary_key=args.dictionary_key,
            remove_unmatched_edges=args.remove_unmatched_edges,
        )
