...
- [x] Wikivoyage German phrasebook source 147（page 12641）：pinned artifact `12641.jsonl`（792 entries，SHA-256 `0f6b35e23873c7ede18a6e6c7b41e871f6acfc33179efe8f01737ef4eca13c38`）經 source-only staging、全量 surface audit（`stats={}`）與本地 fixture 驗證後發布。delta `147-wikivoyage-german-surface-20260914.split.sql`（SHA-256 `8f1f53f4143b5c5eb8c709065adcdaf353fb0bdde52ea94a85468fffd0a8e21c`，23 個 data batches）；operation `6c1bc69554ea46ea999708ecda93f63c`、bookmark `00000282-00000000-000050e5-3a78c518b9eb107da2236ad1349b6085`。postflight source claims `deu=792／eng=793`、edge claims `793`、`deu-Latn-DE` links `792`、source readings `922`（staging 923 中的 `butter` reading 與 Portuguese source 139 共用唯一鍵，production 保留既有 row）、reading slash／德語 surface punctuation／bracket 均為 0；`language_statistics` 已刷新，未拉取 production 全庫。

- [x] Wikivoyage Portuguese phrasebook source 139（page 28280）：pinned artifact `28280.jsonl`（400 entries，SHA-256 `4475d4456884e2a78b58aa91c29531ff87bdf4261dfd82a862bdb765c1c5e2a8`）經 source-only staging、全量 surface audit（`stats={}`）與本地 fixture 驗證後發布。主 delta `139-wikivoyage-portuguese-surface-20260914.split.sql`（SHA-256 `385509d50bba988c05358c05c6bbfda8cfe4126875bbd6bf29c669c39736fc5a`）operation `27c7cce1440c43369481cd9bdfe2c0a1`、bookmark `00000279-00000000-000050e5-29b127d5228067767b36de44c86af00f`；postflight 抓到一條空白斜線殘留後，以 follow-up delta `139-wikivoyage-portuguese-surface-followup-20260914.split.sql`（SHA-256 `7488555c82a0d34850371a34db98cc8d5212be2be2e33bb360adbb5a80ca71e3`）operation `8bcc9adb374b4a78976e6aeb6ef39674`、bookmark `0000027b-00000000-000050e5-2f28455909b4a0cabb1c1df86d7ba9d6` 修正。最終 source claims `eng=402／por=400`、edge claims `402`、readings `737`；slash／leading punctuation／gender note 均為 0，未拉取 production 全庫。
