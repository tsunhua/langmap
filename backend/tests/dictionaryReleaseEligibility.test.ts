...
const EDGE_SQL = 'SELECT id,expression_a_id,expression_b_id,relation_mask,score FROM expression_edges WHERE (expression_a_id IN (?) OR expression_b_id IN (?)) AND (relation_mask & 7) <> 0 ORDER BY id';
