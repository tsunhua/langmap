...
describe('canonical integer schema contract', () => {
  it('uses integer identity keys and canonical language relationships', () => {
    expect(schema).toMatch(/CREATE TABLE languages[\s\S]*?id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY[\s\S]*?code TEXT NOT NULL UNIQUE/s);
    expect(schema).toMatch(/CREATE TABLE language_locales[\s\S]*?id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY[\s\S]*?language_id BIGINT NOT NULL/s);
    expect(schema).toMatch(/CREATE TABLE expressions[\s\S]*?id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY[\s\S]*?language_id BIGINT NOT NULL/s);
    expect(schema).toMatch(/CREATE TABLE expressions[\s\S]*?pos_mask BIGINT NOT NULL DEFAULT 0/s);
  ...
  it('uses compact locale, reading, edge and vote tables', () => {
    expect(schema).toMatch(/CREATE TABLE expression_locale_links[\s\S]*?PRIMARY KEY \(expression_id, locale_id\)[\s\S]*?\);/s);
    expect(schema).toMatch(/CREATE TABLE expression_readings[\s\S]*?PRIMARY KEY \(expression_id, locale_id, scheme, value\)[\s\S]*?\);/s);
    expect(schema).toMatch(/CREATE TABLE expression_edges[\s\S]*?relation_mask BIGINT NOT NULL DEFAULT 1[\s\S]*?annotations_json TEXT NOT NULL DEFAULT '\[\]'/s);
  ...
  it('defines handbook, morphology, split and UI integer foreign keys', () => {
    expect(schema).toMatch(/CREATE TABLE handbooks[\s\S]*?language_locale_id BIGINT/s);
    ...
    expect(schema).toMatch(/CREATE TABLE handbook_sections[\s\S]*?id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY/s);
    expect(schema).toMatch(/CREATE TABLE handbook_section_items[\s\S]*?PRIMARY KEY \(section_id, position\)[\s\S]*?\);/s);
    expect(schema).toMatch(/CREATE TABLE expression_form_edges[\s\S]*?id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY/s);
    expect(schema).toMatch(/CREATE TRIGGER trg_expression_form_edges_no_reverse/);
    expect(schema).toMatch(/CREATE TABLE expression_split_moves[\s\S]*?PRIMARY KEY \(split_id, edge_id\)[\s\S]*?\);/s);
    expect(schema).toMatch(/CREATE TABLE ui_locales[\s\S]*?locale_id BIGINT NOT NULL/s);
    expect(schema).toMatch(/CREATE TABLE ui_messages[\s\S]*?source_expression_id BIGINT NOT NULL/s);
  });

  it('keeps POS bits stable and removes release and packed tables', () => {
    expect(schema).toMatch(/CREATE TABLE parts_of_speech[\s\S]*?bit_index BIGINT NOT NULL UNIQUE[\s\S]*?CHECK \(bit_index BETWEEN 0 AND 62\)/s);
    for (const table of [
