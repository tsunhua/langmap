...
feed.get('/hot', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT ed.id as edge_id, ed.expression_a_id as id, ed.score, ed.source,
      a.id as a_id, a.text as a_text, a.language_profile_code as a_lang,
      b.id as b_id, b.text as b_text, b.language_profile_code as b_lang,
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT * FROM (
       SELECT 'mapping' as type, CAST(ed.expression_a_id AS TEXT) as id,
         ...
         a.text as left_text, a.language_profile_code as left_lang,
         b.text as right_text, b.language_profile_code as right_lang,
       ...
       SELECT 'expression' as type, CAST(e.id AS TEXT) as id,
         ...
         e.text as left_text, e.language_profile_code as left_lang,
