...
handbooks.get('/:id', optionalAuth, async (c) => {
  ...
  const sections=await c.env.DB.prepare('SELECT id,title,position,parent_section_id FROM handbook_sections WHERE handbook_id=? ORDER BY position,id').bind(id).all<any>(); const items=await c.env.DB.prepare('SELECT i.section_id,i.position,e.id,e.text,e.homograph_index,l.code AS lang_code FROM handbook_section_items i JOIN handbook_sections s ON s.id=i.section_id JOIN expressions e ON e.id=i.expression_id JOIN languages l ON l.id=e.language_id WHERE s.handbook_id=? ORDER BY i.section_id,i.position').bind(id).all<any>();
