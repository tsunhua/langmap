...
// visible so standalone example sentences behave like any other expression.
...
const A_SIDE_SQL = `SELECT e.id AS edge_id, n.id AS neighbor_id, l.code AS neighbor_lang_code,
 ...
 WHERE e.expression_a_id = ? AND e.expression_b_id > ? AND (e.relation_mask & ${MAPPING_RELATION_MASK}) <> 0
...
const B_SIDE_SQL = `SELECT e.id AS edge_id, n.id AS neighbor_id, l.code AS neighbor_lang_code,
 ...
 JOIN languages l ON l.id = n.language_id
 WHERE e.expression_b_id = ? AND e.id > ? AND (e.relation_mask & ${MAPPING_RELATION_MASK}) <> 0
 ORDER BY e.id ASC
 LIMIT ?`;
