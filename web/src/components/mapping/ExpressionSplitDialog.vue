...
<template>
  <div class="split-backdrop" role="presentation" @click.self="emit('close')">
    <section class="split-dialog" role="dialog" aria-modal="true" aria-labelledby="split-title">
      <h2 id="split-title">Split expression</h2>
      <p class="warning">Readings, locale attestations and handbook items will not be copied.</p>
      <fieldset><legend>Select mappings to move</legend>
        <label v-for="edge in orderedEdges" :key="edge.edge_id" class="edge-option">
          ...
          <span>{{ edge.neighbor_text }} <small>({{ edge.neighbor_lang_code }}, score {{ edge.score }})</small></span>
        </label>
      </fieldset>
      <p v-if="error" role="alert" class="error">{{ error }}</p>
      <div class="actions"><button class="btn btn-sm" type="button" @click="emit('close')">Cancel</button><button data-action="confirm-split" class="btn btn-primary btn-sm" type="button" :disabled="!selected.size || submitting" @click="confirm">{{ submitting ? 'Splitting…' : 'Split expression' }}</button></div>
    </section>
  </div>
</template>
