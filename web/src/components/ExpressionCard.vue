          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2" v-if="audioList.length > 0 || editable">
              <template v-for="(audioItem, index) in audioList" :key="index">
                <div
                  class="inline-flex items-center bg-slate-50 border border-slate-200 rounded-full transition-colors group/pill h-8">
                  <button @click.stop.prevent="playAudio(audioItem, index)"
                    class="inline-flex items-center justify-center text-slate-600 hover:text-blue-600 hover:bg-slate-200 active:bg-slate-300 focus:outline-none p-1 ml-0.5 rounded-full transition-colors"
                    :title="$t('play')">
                    <svg v-if="playingIndex !== index" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
                  ...
                  <span class="text-xs font-medium text-slate-600 px-1.5 cursor-default truncate max-w-[100px]"
                  ...
                  <div v-if="canDeleteAudio(audioItem.speaker)"
                    class="w-0 overflow-hidden group-hover/pill:w-7 transition-[width] duration-200 ease-in-out flex items-center justify-end h-full pr-1">
                    <button @click.stop.prevent="handleRemoveAudio(audioItem.speaker)"
                      class="flex-shrink-0 flex items-center justify-center text-slate-400 hover:text-red-600 hover:bg-red-100 rounded-full transition-colors w-5 h-5"
              ...
              <template v-if="editable && canAddAudio && item.language_code !== 'image' && item.language_code !== 'emoji'">
                ...
                <button v-else-if="editable && canAddAudio" @click.stop.prevent="showAudioRecorder = true"
                  class="inline-flex items-center text-sm text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-full transition-colors font-medium">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="mt-3 flex flex-wrap gap-1 items-center">
              <span v-for="(tag, index) in getTagsList()" :key="index"
                ...
                <button v-if="editable" @click.prevent="removeTag(tag)"
                  class="ml-1 text-slate-500 hover:text-slate-800 focus:outline-none" title="Remove tag">
