...
export async function listUiLocales(): Promise<UiLocale[]> { const { data } = await api.get(path('/locales'), { params: { _content_revision: contentRevision.value } }); return data.data ?? [] }
