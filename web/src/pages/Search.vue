<template>
  <div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
      <h2 class="text-2xl font-bold text-slate-800 mb-2">{{ $t('search.title') }}</h2>
      <p class="text-slate-600 mb-6">{{ $t('search.description') }}</p>
    ...
    <div v-if="loading" class="flex items-center justify-center py-12">
      ...
      <span class="ml-2 text-slate-600">{{ $t('search.searching') }}</span>
    </div>
      ...
      <div v-else>
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-slate-800">
            {{ $t('search.foundExpressions', { count: items.length, plural: items.length !== 1 ? $t('search.plural') : '' }) }}
          </h3>
        </div>
