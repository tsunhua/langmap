  ...
  <div class="min-h-screen flex items-top justify-center bg-slate-50 py-4 sm:py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <div v-if="!registrationSuccess" class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
        <form class="space-y-6" @submit.prevent="handleRegister">
          <div>
            ...
            <div class="mt-1">
              <input id="username" name="username" type="text" autocomplete="username" required v-model="form.username"
          ...
          <div class="flex items-start">
            <div class="flex items-center h-5">
              <input id="terms" name="terms" type="checkbox" required v-model="form.agreeToTerms"
            ...
            <div class="ml-3 text-sm">
              <label for="terms" class="text-slate-900">
                {{ $t('agree_to_terms') }}
                <router-link to="/policies" class="text-blue-600 hover:text-blue-500">{{ $t('terms_of_service')
                  }}</router-link>
                {{ $t('and') }}
                <router-link to="/policies" class="text-blue-600 hover:text-blue-500">{{ $t('privacy_policy')
                  }}</router-link>
              </label>
            </div>
          </div>
        ...
        <div class="mt-6">
          ...
          <div class="mt-6">
            <router-link
              :to="{ path: '/login', query: { redirect: $route.query.redirect || $router.currentRoute.value.fullPath } }"
      ...
      <div v-else class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 text-center">
        ...
        <div class="mt-6">
          <router-link :to="{ path: '/login', query: { redirect: $route.query.redirect || '/' } }"
            class="w-full flex justify-center py-2 px-4 border border-slate-300 rounded-md shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            {{ $t('go_to_login') }}
          </router-link>
        </div>
