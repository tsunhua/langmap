<template>
   <div class="max-w-7xl mx-auto">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div v-if="meanings.length > 0" class="space-y-4">
          <div class="flex justify-between items-center mb-2">
            ...
            <button
              v-if="!searchOtherExpressionsMode"
              @click="toggleSearchOtherExpressions()"
          ...
          <div v-if="searchOtherExpressionsMode" class="border border-slate-200 rounded-lg bg-slate-50 p-4 mb-4">
            <div class="flex items-center gap-3 mb-3">
              <div class="flex-1">
                <input
                  v-model="searchOtherExpressionsQuery"
            ...
            <div v-else-if="searchOtherExpressionsResults && searchOtherExpressionsResults.length > 0" class="mt-3 space-y-3">
              <div
                v-for="expr in searchOtherExpressionsResults"
                :key="expr.id"
                class="bg-white rounded-lg border border-slate-200 overflow-hidden"
              >
                <div
                  @click="toggleExpressionExpansion(expr.id)"
                ...
                >
                  <div class="flex items-center justify-between">
                    ...
                    <svg
                      ...
                      :class="{ 'rotate-180': expandedExpressionId === expr.id }"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                ...
                <div v-if="expandedExpressionId === expr.id" class="border-t border-slate-200 bg-slate-50 p-3">
                  <div v-if="expr.meanings && expr.meanings.length > 0" class="space-y-2">
                    ...
                    <div
                      v-for="meaning in expr.meanings"
                      :key="meaning.id"
                ...
                <div v-else-if="groupSearchResults[meaning.id] && groupSearchResults[meaning.id].length > 0" class="mt-3 space-y-2">
                  ...
                  >
                    ...
                    <div v-if="c && item && c.id !== item.id && !isLinked(c.id)">
                      <button 
                        @click="openMeaningSelection(c, meaning.id)" 
    ...
    <div v-if="showMeaningSelection && selectedExpressionForAssociation" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-lg max-w-2xl w-full mx-4 max-h-[80vh] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-else-if="expressionMeanings.length > 0" class="space-y-3">
            <div
              v-for="meaning in expressionMeanings"
              :key="meaning.id"
            ...
            >
              <div class="flex justify-between items-center">
                <div>
                  ...
                  <p class="text-sm text-slate-500 mt-1">{{ $t('created_by') }}: {{ meaning.created_by || $t('anonymous') }}</p>
                </div>
