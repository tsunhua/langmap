<template>
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <!-- Empty State -->
    <div v-else-if="handbooks.length === 0" class="text-center py-16 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
      ...
      >
        <div class="p-6 flex-grow">
          <div class="flex justify-between items-start mb-3">
            ...
            <span v-if="handbook.is_public" class="bg-green-100 text-green-800 text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full">
              {{ $t('public') }}
            </span>
          </div>
    ...
    <!-- Pagination -->
    <div v-if="currentPage > 1 || handbooks.length === itemsPerPage" class="flex justify-center items-center gap-4 mt-12 pb-8">
      <button 
        @click="prevPage" 
      ...
      <button 
        @click="nextPage" 
        :disabled="handbooks.length < itemsPerPage"
        class="px-5 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed text-sm font-medium transition-colors"
      >
        {{ $t('next') }} &raquo;
      </button>
