...
- **mirror 只建立一次、之後增量維護**：首次以 `wrangler d1 export --remote` 快照灌進 fresh SQLite（使 ID 空間與 production 一致），之後每部導入 `--d1 <mirror>` → `export_dictionary_delta.py` 產 `INSERT OR IGNORE` SQL → `manage.sh production plan --approved-data-migration <delta>` → `production apply`（bookmark 後執行）→ **apply 成功後把同一份 delta replay 回 mirror**，維持兩邊一致（`sqlite_sequence` 隨 max(id) 自動對齊）。不可用 `./dev.sh --rebuild` 的 generator seed（ID 空間不一致）；也不需每次全量 export（遠端庫持續變大，下載只會更慢）。
- **僅在下列情況重新全量 export**：restore-from-bookmark（production 倒回，mirror 需同步）、mirror 與 production count 不一致、或 mirror 遺失。每次新增 role：先 `python3 -m pytest scripts/db/tests scripts/dictionary/tests -q` 確認既有測試與 delta/replay 邏輯仍綠。
- 每部流程：`import_with_progress.py --d1 <mirror>` → `scripts/db/export_dictionary_delta.py` 產 `INSERT OR IGNORE` SQL → `manage.sh production plan --approved-data-migration <delta>` → `production apply`（bookmark 後執行）→ replay delta 回 mirror。
