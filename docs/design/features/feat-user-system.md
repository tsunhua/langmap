...
**设计来源**：本设计基于原始 `design_user.md` 和系统架构设计文档

**实现状态**：
- ✅ 用户基础认证（注册、登录、登出）- 已实现
- ✅ JWT 认证机制 - 已实现（Hono + jose）
- ✅ 用户数据模型（users 表）- 已实现
- ⏳ 邮箱验证功能 - 设计已定义，实现状态需确认
- ⚠️ 用户角色管理（更新角色、分配权限）- 部分实现（仅超级管理员角色定义）
- ❌ 修订审核队列 - 未实现（revisions 表未创建）
- ❌ 两步验证 - 未实现

**未实现或需确认的功能**：
- `POST /api/v1/users/:id/role` - 更新用户角色（仅超级管理员）
- `PUT /api/v1/expressions/:id/revision/:revision_id/approve` - 审核内容修改
...
本文档描述了 LangMap 应用程序中用户管理和权限控制系统的设计。该系统包含三种用户角色：超级管理员、管理员和普通用户，每种角色具有不同的权限级别。
