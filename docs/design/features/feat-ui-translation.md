...
**设计来源**：本设计整合了原始 `design_ui_translation.md` 和 `user_translate_locales_design.md`
...
- ❌ 审核工作流完善 - 未实现（审核队列、批准/拒绝功能）
- ❌ 高级工具 - 未实现（机器翻译建议、翻译记忆、术语表）
- ❌ 质量保证 - 未实现（翻译完整性验证、复数形式、上下文信息）

**未实现或需确认的 API 端点**：
- `POST /api/v1/ui-translations/:language` - 保存翻译（部分实现，需确认批量保存功能）
- 完成度计算 API（前端已实现，后端可能需要对应端点）
- 语言自动激活 API（需要确认是否已实现）
...
UI 翻译系统设计，包括用户翻译界面、本地化翻译同步方案和翻译数据管理机制。
