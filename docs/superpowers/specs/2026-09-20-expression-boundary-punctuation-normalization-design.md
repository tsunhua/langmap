...
左右不同的符號使用 stack 配對。左右相同的 ASCII 引號按出現順序成對。位於文字內部、且
至少一側緊鄰字母或數字的 `'` 與 `’` 視為 lexical apostrophe，不參與引號計數，例如
`don't` 與詞典中的 `Abu' Arapesh` 保持原樣；位於句首或句尾的 apostrophe 仍按引號處理。
