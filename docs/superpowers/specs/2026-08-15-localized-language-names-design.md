...
LangMap 的介面語系可以切換至 `cmn-Hans-CN`、`cmn-Hant-TW` 等 Language Locale，但目前多數 API 直接以 `languages.name_en` 回傳語言名稱。結果是使用者切換至 `cmn-Hans-CN` 後，Mapping Detail 仍顯示 `jpn Japanese`，而非 `jpn 日语`。

LangMap 已使用 expression、`expression_edges`、locale attestation 與 vote score 維護 UI 文案翻譯。語言名稱應沿用同一套詞句 mapping，而不是在前端硬編碼名稱，或為每個 UI locale 在 `languages` 增加欄位。

## 2. 目標

- 語言名稱隨目前 primary／secondary UI locale 在全站一致改變。
- `jpn` 在 `cmn-Hans-CN` 下顯示 `日语`；沒有合格譯名時安全回退至 `Japanese`。
- 語言名稱的跨語言關係由既有 expression mapping、locale attestation 與 vote score 維護。
...
- 不把全部語言名稱加入 `langmap-web` 的 UI message bundle。
...
`name_en` 暫時保留，作為既有 API 相容欄位、seed 可讀資料及解析失敗時的英文回退。`name_expression_id` 才是進入多語 mapping 的穩定入口。
...
新增一個增量 migration，並同步更新 `backend/schema.sql`。遷移需為現有 language rows 建立或重用 canonical 英文名稱 expression，再回填 `name_expression_id`。

遷移完成後允許極少數未能回填的 legacy row 暫時保留 `NULL`；解析器直接回退 `name_en`。新建或由正式 seed／同步流程管理的 language row 必須填入 `name_expression_id`。

更新語言 registry 的 seed、同步與驗證流程，確保未來新增語言時名稱 expression 與引用同步建立。歷史 migration 不回寫。

## 5. 共用名稱解析器

後端新增一個批次解析服務，以一組 language code、primary locale 與可選 secondary locale 為輸入，輸出 `code → localized name`。所有 consumer 必須使用此服務或使用已由它解析的 API 欄位，不在 route 或前端重複 SQL／回退邏輯。

### 5.1 合格候選

對每個 `languages.name_expression_id`，候選名稱必須同時符合：
...
圖譜與列表必須批次解析該回應內 distinct language codes，不得對每個節點或列執行獨立查詢。解析查詢需有明確數量上限，沿用呼叫端既有 page／node limit，並以穩定排序取得候選。
...
所有回傳語言顯示名稱的公開 API 接受目前 primary UI locale；支援 secondary locale 的 consumer 同時傳入 secondary。參數命名在全站統一為：
...
既有 `language_name` 或 language summary 的 `name` 改為解析後名稱；`name_en` 保留英文原值。language code 與 expression identity 不受名稱本地化影響。
...
前端不建立獨立的 language-name 翻譯表，避免與 mapping 資料分叉。

## 7. 寫入與 revision

新增、刪除或投票改變語言名稱相關 edge 時，名稱解析應在下一次請求反映最新 score。若未來為讀取結果加入 cache，cache key 至少包含 language code、primary locale、secondary locale 與受影響 locale 的 mapping revision。

本次可以先採每次請求解析，不預先建立另一套 revision table。不得因效能推測提前複製名稱譯文。

## 8. 錯誤處理

- `name_expression_id` 缺失或引用資料不可用：回退 `name_en`，記錄可觀測錯誤，不使整個頁面失敗。
...
- primary 無候選時使用 secondary；兩者都無候選時回退 `name_en`。
- 負分 edge 不採用。
...
- 語言名稱的翻譯只由 canonical name expression、direct mapping、locale attestation 與 score 決定。
- 全站所有已顯示語言名稱的 consumer 使用同一解析規則。
