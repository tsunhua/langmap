...
內容語言採「共通層 → 變體層」。基礎 BCP 47 script profile（例如 `en-Latn`、
`ko-Hang`、`yue-Hant`）承載已確認共通或尚未分類的詞句；帶 region 或 private-use
身份的 profile（例如 `en-Latn-US`、`nan-Hant-x-chao1238`）只承載已確認的
變體內容。韓語以 script profile（`ko-Hang`、`ko-Hani`、`ko-Kore`）取代地區
profile；日本語以 `ja-Jpan`（標準假名＋漢字混用）、`ja-Hani`、`ja-Hrkt`、
`ja-Latn` 取代裸碼 `ja`；其餘語言同步由裸碼改為 script profile（例如
`en-Latn`、`es-Latn`、`ar-Arab`、`hi-Deva`、`ru-Cyrl`）。

UI locale 是另一個概念，但第一方英文介面沒有使用美國特有規則，因此來源 locale
採 `en-Latn`。其他確有 script／locale 需求的 UI locale 可保留更完整的 tag。
...
- `en-Latn`、`pt-Latn`、`ko-Hang`、`ja-Jpan` 是共通／待分類內容入口；
  `en-Latn-US`、`en-Latn-GB`、`pt-Latn-BR` 繼續存在以記錄已確認差異。
  韓語以 `ko-Hang`／`ko-Hani`／`ko-Kore` 三個 script profile 取代原有的
  `ko`／`ko-KR`／`ko-KP`；日本語以 `ja-Jpan`／`ja-Hani`／`ja-Hrkt`／`ja-Latn`
  取代裸碼 `ja`。
...
- 不把內容 profile `en-Latn-US` 刪除；它仍可承載美國英語特有詞句。

## 6. 驗收

- registry 同時含 `en-Latn`／`en-Latn-US`／`en-Latn-GB`、
  `pt-Latn`／`pt-Latn-BR`、`ko-Hang`／`ko-Hani`／`ko-Kore`、
