...
- Final commits: LangMap `06a514f2` (`refactor: remove D1 staging and use wide UI CSV`) and dictionary `834eaf3` (`refactor: retire dictionary JSONL outputs`). The two ignored legacy dictionary JSONL exports were removed; raw payloads and the 17-GB local database state remain uncommitted by design.
